# sails-hook-autoreload-offshore

Based on the work of [sails-hook-autoreload](sgress454/sails-hook-autoreload).

[Sails JS](http://sailsjs.org) hook to autoreload controllers, models, services and locales when changed. Working with the [Offshore ORM](https://github.com/Atlantis-Software/offshore).
...
`npm install sails-hook-autoreload-offshore`
...
> `sails-hook-autoreload-offshore` will, by default, use the `migrate: alter` strategy when reloading model files unless you explicitly set the `overrideMigrateSetting` config to `false`.  This allows you to change models without losing your development data when you have `migrate: drop` set.
